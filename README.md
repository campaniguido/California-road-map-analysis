...
In any case these scripts can be used on any dataset representing a network as a couple of edges linked together.
...
```
All codes must be executed inside a python environment, thus it could be necessary to add `python` before the instructions shown. Before all the simulations some parameters need to be set. They are stored in the files `parameters.py` and `parameters_Copycat.py`. In these files default values are already given.
...
### The California road map and ERG simulations (road_map.py)
